...
export function PreviewFrame() {
  ...
  const sendDarkMode = useCallback((dark: boolean) => {
    iframeRef.current?.contentWindow?.postMessage({ type: 'SET_DARK_MODE', dark }, '*');
  }, []);
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <button
        ...
        className="absolute top-3 right-3 p-2 rounded-lg bg-white/80 hover:bg-white border border-neutral-200 shadow-sm backdrop-blur-sm transition-colors"
        title={darkMode ? "Switch to light mode" : "Switch to dark mode"}
      >
        {darkMode ? <Sun className="w-4 h-4 text-neutral-700" /> : <Moon className="w-4 h-4 text-neutral-700" />}
      </button>
    </div>
  );
}
